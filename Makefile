...
RUN ?= $(if $(HAS_DOCKER), docker run $(DOCKER_ARGS) --shm-size 12G --rm -e AWS_ACCESS_KEY_ID=$(AWS_ACCESS_KEY_ID) -e AWS_SECRET_ACCESS_KEY=$(AWS_SECRET_ACCESS_KEY) -v $$(pwd):/code -w /code -u $(UID):$(GID) $(IMAGE))
